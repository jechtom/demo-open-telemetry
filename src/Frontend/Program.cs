...
var (protocol, uri) =
    (
        builder.Configuration.GetValue<Uri?>("OTEL_GRPC_ENDPOINT"),
        builder.Configuration.GetValue<Uri?>("OTEL_HTTP_ENDPOINT")
    ) switch 
    {
        (Uri u, _) => (OtlpExportProtocol.Grpc, u),
        (_, Uri u) => (OtlpExportProtocol.HttpProtobuf, u),
        _ => throw new InvalidOperationException("Invalid OTEL config.")
    };

Console.WriteLine($"OTEL Endpoint: {protocol} at {uri}");

// configure OpenTelemetry
builder.Services.AddOpenTelemetry()
   .ConfigureResource(r => r.AddService("Frontend", serviceVersion: "1.2.3.4"))
   .UseOtlpExporter(protocol, uri)
...
{
    c.BaseAddress = new Uri("http://order-service:8080");
});

builder.Services.AddHttpClient<DiceClient>(c =>
{
    c.BaseAddress = new Uri("http://dice-service:8080");
});
